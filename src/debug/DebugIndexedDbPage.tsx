...
export function DebugIndexedDbPage() {
  ...
  const onExport = useCallback(async () => {
    ...
    try {
      ...
      setStatus(
        `Exported ${snapshot.learningCards.length} learning card(s), ${snapshot.studentMoods.length} mood row(s), ${snapshot.users.length} user(s), ${snapshot.reports.length} report(s), ${snapshot.broadcasts.length} broadcast(s), ${snapshot.quizzes.length} quiz worksheet(s).`,
      );
    } catch (e) {
      console.error(e);
